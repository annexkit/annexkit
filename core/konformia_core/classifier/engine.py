...
from konformia_core.classifier.annex import ANNEX, Rule

RiskTier = Literal["unacceptable", "high", "limited", "minimal"]
RuleType = Literal["prohibited", "high_risk", "transparency"]

# The risk tiers as a runtime tuple — for schema validators and DB constraints
# that need to iterate them. Derived from ``RiskTier`` so the two never drift.
VALID_RISK_TIERS: tuple[RiskTier, ...] = get_args(RiskTier)
